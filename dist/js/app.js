(() => {
    ...
    document.addEventListener("mouseover", (function(e) {
        ...
        if (e.target.closest(".actions-footer__language")) languageButtonFooter.classList.add("_search-active"); else languageButtonFooter.classList.remove("_search-active");
    }));
    ...
    function buttonClickSearch(e) {
        if (e.target.closest(".actions-header__search-button")) {
            ...
            if ("" == searchInput.value) e.preventDefault(e);
        }
        if (!e.target.closest(".actions-header__search-input") && !e.target.closest(".actions-header__search-button")) searchInput.classList.remove("_search-active");
    }
    ...
    document.querySelector(".header-submenu__title");
    document.querySelector(".footer-submenu__title");
    document.querySelector(".header-submenu__body");
    document.addEventListener("mouseover", subMenuShow);
    document.addEventListener("click", subMenuShowClick);
    function subMenuShow(e) {
        if (e.target.closest(".header-submenu__title") || e.target.closest(".header-submenu__body")) subMenu.classList.add("submenu-show"); else subMenu.classList.remove("submenu-show");
    }
    function subMenuShowClick(e) {
        if (e.target.closest(".header-submenu__title")) subMenu.classList.toggle("submenu-show");
        if (e.target.closest(".footer-submenu__title")) subMenuFooter.classList.toggle("submenu-show");
    }
    ...
    window.addEventListener("load", (function(e) {
        ...
        window.addEventListener("scroll", (function(e) {
            if (window.scrollY > sticky) header.classList.add("sticky"); else header.classList.remove("sticky");
        }));
    }));
    ...
    function getImages(className) {
        ...
        for (i = 0; i < children.length; i++) {
            ...
            for (j = 0; j < classNames.length; j++) if (classNames[j] == className) {
                elements.push(child);
                break;
            }
        }
    ...
    function getClasses(classes, string) {
        var temp = "";
        for (var j = 0; j < classes.length; j++) if (classes[j] != string) {
            if (temp) temp += " ";
            temp += classes[j];
        }
        return temp;
    ...
    function getClassValue(classes, string) {
        ...
        for (var j = 0; j < classes.length; j++) if (0 == classes[j].indexOf(string)) {
    ...
    function addIEReflex() {
        ...
        var i, j, z, q, dist, iter, rest, higt, divs, opac, colr, bord, wide, half, ih, iw, ww, hh, fb, xb;
        ...
        for (i = 0; i < theimages.length; i++) {
            ...
            if (image.width >= 32 && image.height >= 32) {
                ...
                if (true == itiltright) tilt = "r";
                if (true == itiltnone) tilt = "n";
                if (true == itiltleft) tilt = "l";
                newClasses = getClasses(classes, "reflex");
                ih = image.height;
                iw = image.width;
                dist = idistance;
                Math.min(iradius, Math.max(iw, ih) / 10);
                colr = 0 != icolor ? icolor : "#000000";
                opac = iopacity > 0 ? iopacity : 33;
                divs = 100 / (iheight >= 10 ? iheight : 33);
                (iheight >= 10 ? iheight : 33) / 100;
                higt = Math.floor(ih / divs);
                wide = 12;
                if (1 == iborder) bord = 0; else {
                    iborder = 2 * Math.floor(Math.round(Math.min(Math.min(iborder, higt / 4), Math.max(iw, ih) / 20)) / 2);
                ...
                hh = iter + (rest > 0 ? 1 : 0);
                z = (ih - hh - hh) / ih;
                display = "block" == image.currentStyle.display.toLowerCase() ? "block" : "inline-block";
                ...
                display = "left" == flt || "right" == flt ? "inline" : display;
                ...
                if ("r" == tilt) tilt = "n"; else if ("n" == tilt) tilt = "l"; else if ("l" == tilt) tilt = "r";
    ...
    function addReflex() {
        var theimages = getImages("reflex");
        var image, object, canvas, context, classes, newClasses, resource;
        var i, j, dist, stl, iter, rest, higt, divs, opac, colr, bord, wide, ih, iw;
        var itiltright, itiltnone, itiltleft, iheight, iopacity, idistance, iborder, icolor, iradius;
        document.getElementsByTagName("img");
        var tilt = "r";
        var tmp = -1 != navigator.userAgent.indexOf("WebKit") ? true : false;
        var isWK = true == tmp ? window.postMessage ? false : window.external ? false : true : false;
        var isCR = true == tmp && window.external ? true : false;
        for (i = 0; i < theimages.length; i++) {
            image = theimages[i];
            object = image.parentNode;
            tmp = 0;
            ...
            if (canvas.getContext && image.width >= 32 && image.height >= 32) {
                ...
                if (isWK) {
                    ...
                    context.closePath();
                    context.fill();
                } else context.fillRect(0, canvas.height - higt, canvas.width, higt);
                context.globalCompositeOperation = "source-over";
                context.drawImage(image, 0, 0, iw, ih);
                context.save();
                if (isWK && dist > 0 && "n" != tilt) {
                    context.fillStyle = "#808080";
                    context.fillRect(0, canvas.height - higt - dist, canvas.width, dist);
                }
                if (iborder > 0) if ("n" == tilt) {
                    context.beginPath();
                    context.rect(bord, bord, canvas.width - iborder, canvas.height - higt - dist - iborder);
                    context.closePath();
                    context.stroke();
                }
                if ("l" == tilt || "r" == tilt) if (resource.getContext) {
                    context = resource.getContext("2d");
                    context.globalCompositeOperation = "source-over";
                    context.clearRect(0, 0, resource.width, resource.height);
                    if ("r" == tilt) {
                        for (j = 0; j < iter; j++) context.drawImage(canvas, j * wide, 0, wide, resource.height, j * wide, 1 * j, wide, resource.height - 2 * j);
                        if (rest > 0) {
                            rest = canvas.width - iter * wide;
                            context.drawImage(canvas, j * wide, 0, rest, resource.height, j * wide, 1 * j, rest, resource.height - 2 * j);
                        }
                    } else {
                        for (j = 0; j < iter; j++) context.drawImage(canvas, j * wide, 0, wide, resource.height, j * wide, 1 * (iter - j), wide, resource.height - 2 * (iter - j));
                        if (rest > 0) {
                            rest = canvas.width - iter * wide;
                    ...
                    if (canvas.getContext) {
                        context = canvas.getContext("2d");
                        context.clearRect(0, 0, canvas.width, canvas.height);
                        if ("r" == tilt) clipPolyRight(context, canvas.width / 20, 0, .95 * canvas.width, canvas.height, iter + (rest > 0 ? 1 : 0), dist, higt); else clipPolyLeft(context, canvas.width / 20, 0, .95 * canvas.width, canvas.height, iter + (rest > 0 ? 1 : 1), dist, higt);
                        context.clip();
                        context.drawImage(resource, parseInt(canvas.width / 20), 0, parseInt(.9 * canvas.width), canvas.height);
                        context.save();
                        if (iborder > 0) {
                            context.lineWidth = iborder;
                            if ("r" == tilt) {
                                strokePolyRight(context, canvas.width / 20, 0, .95 * canvas.width, canvas.height, iter + (rest > 0 ? 1 : 0), dist, higt, bord);
                                context.stroke();
                            } else {
                                strokePolyLeft(context, canvas.width / 20, 0, .95 * canvas.width, canvas.height, iter + (rest > 0 ? 1 : 0), dist, higt, bord);
                                context.stroke();
                            }
                        }
                        if (isWK) {
                            context.globalCompositeOperation = "destination-out";
                            stl = context.createLinearGradient("l" == tilt ? canvas.width : 0, canvas.height - higt, "l" == tilt ? canvas.width - parseInt(wide / divs) : parseInt(wide / divs), canvas.height);
                            stl.addColorStop(1, "rgba(255,0,0,1.0)");
                            stl.addColorStop(0, "rgba(255,0,0," + opac + ")");
                            context.fillStyle = stl;
                            clipReflex(context, canvas.width / 20, 0, .95 * canvas.width, canvas.height, iter + (rest > 0 ? 1 : 0), dist, higt, tilt);
                            context.fill();
                            globalCompositeOperation = "source-in";
                            clearReflex(context, canvas.width / 20, 0, .95 * canvas.width, canvas.height, iter + (rest > 0 ? 1 : 0), dist, higt, tilt);
                            context.clip();
                            context.clearRect(0, 0, canvas.width, canvas.height);
                            context.clearRect(0, 0, canvas.width, canvas.height);
                            context.clearRect(0, 0, canvas.width, canvas.height);
